...
В первую очередь нужно создать набор тайлов (tileset). Ниже представлены изображения, которые мы будем использовать. По соображениям оптимизации все они одинаковы (но могут принимать разную форму: прямоугольниги, шестигранники, паралеллограммы - прим. пер.). Благодаря "упаковке" мы можем собрать воедино несколько спрайтов и получить палитру тайлов.
...
Также установите размер ячеек `50` (это размер тайлов). Quadrant (квадрант) - это настраиваемое значение, которое указывает сколько плиток должен отрисовывать движок и выводить на видимую область экрана, по умолчанию оно равно `16х16`. Это значение изначально настроено правильно и не нуждается в изменении, но в определенных случаях, для точной настройки производительности можно прибегнуть к редактированию этого параметра (если вы знаете, что делаете).
...
* Масштабирование  через `Viewport` часто помогает с сокращением карты. Простое добавление камеры, настройка ее Current и игра с ней Zoom могут быть хорошей отправной точкой.

* Вы можете использовать отдельно взятое изображение для каждой плитки. Это позволит удалить все артефакты, но данный способ очень затратен по времени и менее оптимизирован.