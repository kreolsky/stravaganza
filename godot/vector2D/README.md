...
Скалярное произведение - это операция двух векторов, результатом которой является скаляр (произведение величин этих векторов, умноженное на косинус угла между ними). Скаляр является одним из наиболее важных понятий в векторной математике, но часто понимается неправильно. В отличие от вектора, который содержит длину и направление, скалярное значение содержит только длину.
...
### Поле зрения
Мы можем использовать эффект «поле зрения», чтобы определить, обращен ли объект лицом к другому объекту. На приведенном ниже рисунке игрок `P` пытается уйти от зомби `A` и `B`. Предположим, что поле зрения зомби `180°`, могут ли они увидеть игрока?
